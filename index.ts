  ...
  switch (type) {
    case "log":
      return console.info(
        c.greenBright(
          `[ ${setor} | ${
            moment.tz(Date.now(), "America/Bahia").format("LT")
          } ] - ${message}`,
        ),
      );
    case "error":
...
async function starterMenu() {
  ...
  ◆ ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬ ❴ ✪ ❵ ▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬ ◆\n
    ...
    type: "number",
    validate: (option) =>
      option >= 0 && option <= 3 ? true : "Escolha uma opção válida",
  });
  console.clear();
  switch (starterMenu.value) {
    case 0:
      return process.emit("SIGINT", null);
    case 1:
      DISCORD_TOKEN = process.env.CLIENT_DISCORD_TOKEN;
      return start();
    case 2:
      DISCORD_TOKEN = process.env.BETA_CLIENT_DISCORD_TOKEN;
      return start();
    case 3:
      return logsMenu();
  }
...
async function logsMenu() {
  ...
  ◆ ▬▬▬▬▬▬▬▬▬▬▬▬▬ ❴ ✪ ❵ ▬▬▬▬▬▬▬▬▬▬▬▬▬ ◆\n
    ...
    type: "number",
    validate: (option) =>
      option >= 0 && option <= 2 ? true : "Escolha uma opção válida",
  });
  console.clear();
...
function start() {
  ...
  console.log(
    c.bgBlueBright("[ SHARDING MANAGER ] - Iniciando fragmentação..."),
  );
  const ket = new KetClient(
    `Bot ${DISCORD_TOKEN}`,
    settings.ERIS_LOADER_SETTINGS as Eris.ClientOptions,
  );
...
process.on("SIGINT", async () => {
  ...
  try {
    ...
    global.log(
      "error",
      "DATABASE",
      `x Houve um erro ao encerrar a conexão com o banco de dados:`,
      e,
    );
  } finally {
    process.exit();
...
process.on(
  "unhandledRejection",
  (reason, p) => global.log("error", "ANTI-CRASH", `SCRIPT REJEITADO:`, reason),
);

process.on(
  "uncaughtException",
  (err, o) => global.log("error", "ANTI-CRASH", `CATCH ERROR:`, err),
);

process.on(
  "uncaughtExceptionMonitor",
  (err, o) => global.log("error", "ANTI-CRASH", `BLOQUEADO:`, err),
);

process.on(
  "multipleResolves",
  (type, promise, reason) =>
    global.log("error", "ANTI-CRASH", `MULTIPLOS ERROS:`, promise),
);