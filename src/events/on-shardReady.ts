    ...
    async start(shardID) {
        ...
        async function infoEmbed(shardID, ket) {
            ...
            ket.shards.forEach(s => { let status = 'unknown'
                switch(s.status) { case 'ready': status = `online ${emoji('online')}`
                        break 
                    case 'connecting': status = `conectando ${emoji('idle')}`
                        break
                    case 'disconnected': status = `offline ${emoji('offline')}`
                }
                embed.fields.push({ name: `${emoji('cristal')} ${s.id}`,
