...
module.exports = class userfake extends Command {
    ...
    public async execute(ctx: CommandContext) {
        if (!ctx.args[2]) return ctx.send({ content: { embeds: [ctx.noargs] } });

        let user = await this.ket.findUser(ctx.args[0], false) as User,
        ...
        if (this.ket.webhooks.has(ctx.cID)) w = this.ket.webhooks.get(ctx.cID);
            ...
            if (webhooks[0] && webhooks.find(w => w.user.id === this.ket.user.id))
                w = webhooks.find(w => w.user.id === this.ket.user.id)
        ...
        if (ctx.env instanceof Message) ctx.env.delete()
            .catch(() => { });
