...
const
    { Client } = require('eris'),
    { readdir } = require('fs'),
    EventHandler = require('./components/EventHandler'),
    LocalesStructure = require('./components/LocalesStructure');
        ...
        categories.forEach((category) => {
          readdir(`${__dirname}/commands/${category}/`, (e, files) => {
            files.forEach(async (command) => {
              const comando = require(`${__dirname}/commands/${category}/${command}`);
              this.commands.set(comando.config.name, comando);
              return comando.config.aliases.forEach((aliase) =>
                this.aliases.set(aliase, comando.config.name)
              );
            });
          });
        });
    ...
    try {
      readdir(path, (e, files) => {
        files.forEach((fileName) => {
          ...
          this.events.add(
            fileName.split(".")[0].replace("on-", ""),
            `${fileName}_EVENT`,
            `${__dirname}/events/${fileName}`,
            this,
          );
        });
      });
  ...
  loadModules() {
    try {
        ...
        categories.forEach((category) => {
          readdir(`${__dirname}/packages/${category}/`, (e, modules) => {
            modules.forEach(async (file) => {
              ...
              const module = require(`${__dirname}/packages/${category}/${file}`)
      ...
      global.log(
        "error",
        "MODULES MANAGER",
        "Houve um erro ao carregar os módulos:",
        e,
      );
    }
    return this;
