import { TRUSTED_BOTS, guilds, DEFAULT_PREFIX, DEFAULT_LANG } from "../../JSON/settings.json";
...
module.exports = class MessageCreate extends Event {
    ...
    public async on(message: Message<any>): Promise<void> {
        ...
        if (!message.guildID || message.channel.type === 1 || message.channel.parentID === guilds.dmCategory)
            return DMexec(message, this.ket, message.channel.parentID === guilds.dmCategory), void 0;
        ...
        let args: string[] = message.content.replace(prefixRegex, '').trim().split(' '),
